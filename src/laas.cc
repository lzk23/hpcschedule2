...
LaaS::LaaS(std::vector<int> M, std::vector<int> W, std::string logFileName, Energy *energy)
{
  isBadState = false;
  // initialize the tree
  if (M.size() < 2) {
  ...
  ft  = new ThreeLevelFatTree(M[0], M[1], M[2], W[1], W[2]);//构建树
  if (!ft) {
                     ...
                     std::vector<std::pair<int, int> > &l2UpPorts)  // OUT allocated L2 switch up ports
{
  ...
  map<int, map< int, list< int > > >::const_iterator jI;//迭代类型是map<int, map< int, list< int > > >
  jI = alg->L1DnPortsByJob.find(tenantId);
  if (jI != alg->L1DnPortsByJob.end()) {
    map< int, list< int > >::const_iterator sI;
    for (sI = (*jI).second.begin(); sI != (*jI).second.end(); sI++) { //first -> key, second -> value
      int swIdx = (*sI).first;
...
int
LaaS::allocTenant(Job* _job)  // IN number of hosts per each leaf group index
{
  int tenantId = _job->id;
  ...
  unsigned int N = _job->numHosts;
  
  alg->eflag = false;
  bool placeSucceed = alg -> placeJob(_job);
  // if(placeSucceed){
  //   log << "place job "<< _job->id << " succeed with energy" << endl;
  // }
  // if(!placeSucceed){
    
  //   alg->eflag = false;
  //   placeSucceed = alg->placeJob(_job);
  // }
  if (placeSucceed) {
    Job job(tenantId, N);
...
// remove a tenant
int
LaaS::deallocTenant(Job* job, vector<int> &deallohosts)
{
  map<int, Job>::const_iterator jI = jobByID.find(job->id);
  if (jI == jobByID.end()) {
    lastErrorMsg << "Could not find job:" << job->id;
    return 1;
  }
  jobByID.erase(job->id);
  return(alg->completeJob(job, (*jI).second.numHosts, deallohosts) == true);
}
                   ...
                   std::vector<pair<int, int> > &l2UpPorts) // IN allocated L2 switch up ports
{
  ...
  for (size_t i = 0; i < l1UpPorts.size(); i++) {
    ...
    if (alg->L1FreeUpPorts.size() <= l1Idx) {
                     ...
                     std::vector<pair<int, int> > &l2UpPorts) // OUT unallocated L2 switch up ports
{
  ...
  for (size_t i = 0; i < alg->L1FreeUpPorts.size(); i++) {
    ...
    for (size_t j = 0; j < nSet; j++) {
      l1UpPorts.push_back(pair<int, int>(i,setBits[j]));
    }
  }
...
void
LaaS::updateIdleNum(std::vector<int> hostids, Energy* energy){
  for(auto id : hostids){
    int subtreeidx = ft->hostid2subtreeidx(id);
    if(!energy->inlock(id))
      ft->L1SubTreeByIdx[subtreeidx].numIdleHosts--;
    if(ft->L1SubTreeByIdx[subtreeidx].numIdleHosts<0){
      throw "error";
    }
  }
}
...
void
RunningJobs::insert(int t, Job* job)
{
  job->start = t;
  if(jobsByTime.find(t)!=jobsByTime.end()){
    jobsByTime[t].push_back(job);
  }
  else{
    jobsByTime[t] = std::vector<Job*>();
    jobsByTime[t].push_back(job);
    sorted_times.push(t);
  }
}
...
RunningJobs::pop(int &t)
{
  ...
  t = sorted_times.top();
  sorted_times.pop();
  std::vector<Job*> jobs = jobsByTime[t];
  jobsByTime.erase(t);
  return jobs;
}

void
JobReservations::insert(int t, Reservation* job)
{
  //job->start = t;
  if(reservationsBytime.find(t)!=reservationsBytime.end()){
    reservationsBytime[t].push_back(job);
  }
  else{
    reservationsBytime[t] = std::vector<Reservation*>();
    reservationsBytime[t].push_back(job);
    sorted_times.push(t);
    sorted_times_descending.push(t);
  }
}

std::vector<Reservation*>
JobReservations::pop(int &t)
{
  if(sorted_times.size()==0){
  ...
  t = sorted_times.top();
  sorted_times.pop();
  std::vector<Reservation*> reservations = reservationsBytime[t];
  reservationsBytime.erase(t);
  return reservations;
}
