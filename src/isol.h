...
class IsolAlgo {
 ...
 public:
  ...
  IsolAlgo(string o, Energy* energy);

  // return true if placement succeeded
  bool placeJob(Job* job);
  bool completeJob(Job* job, int numHosts, vector<int> &deallo_hosts);
  int setFatTree(ThreeLevelFatTree *ft);
  string jobSettingStr(int);

  friend class LaaS; // assible to private members
  friend class Energy;
};
