...
IsolAlgo::IsolAlgo(string o, Energy *e) 
{
  noLinkAlloc = false;
...
IsolAlgo::setFatTree(ThreeLevelFatTree *f)
{
  ...
  fullL1DnMask.setAll();
  L1FreeDnPorts.resize(ft->M3 * ft->M2, fullL1DnMask); //改变容器大小
...
bool
IsolAlgo::placeJob(Job* job) {
  int A, B, C, D, R, S, Q;
  int numHosts = job->numHosts;
  extern int g_uselesstree;
  if(g_uselesstree){
    numHosts = ((numHosts-1)/ft->M1+1)*ft->M1;
  }
  int jobId = job->id;
  if (numHosts <= 0) 
    return true;
  ...
  if (!possiblePlacement(numHosts, l1Allocations, l2Allocations, 
    ...
    //cout << "-Place Fail- job:" << jobId << ", req:" << numHosts << ", ava:" << availableHosts << endl;
  ...
  recordAllocation(job, l1Allocations, l2Allocations, jobL2Ports, jobL3Ports,
                   numLinks);
  if (verbose) {
    dumpL3();
    dumpL2();
                               ...
                               PortMask &UM) // output unique ports
{
  ...
  for (int i = l; i < le; i++) {
    ...
    if (&ft->L1SubTreeByIdx == H) {
      if (eflag&&(*H)[i].numIdleHosts < A)
        continue;
      if(!eflag&&(*H)[i].numFreeHosts < A)
        continue;
                         ...
                         PortMask    &UM) // unique ports mask
{
  ...
  for (int l = l0; l < le; l++) {
    ...
    // must have enough hosts
    if (&ft->L1SubTreeByIdx == H) {
      if (!eflag&&((*H)[l].numFreeHosts < B))
        continue;
      if (eflag&&((*H)[l].numIdleHosts)<B)
        continue;
    } else {
      // for L2 subtrees we want complete suntrees so count them
      int numFreeL1s = 0;
                          ...
                          vector<PortMask> &jobL1UpPorts)
{
  ...
  for (l2I = L2ByFreeSpace.begin(); l2I != L2ByFreeSpace.end(); l2I++) {
    SubTree *st2 = *l2I;
    if (!eflag&st2->numFreeHosts < N) 
      continue;
    if (eflag&st2->numIdleHosts < N)
      continue;
                          ...
                          vector<PortMask> &jobL2UpPorts)
{
  ...
  for (I = repeatedLeafs.begin(); I != repeatedLeafs.end(); I++) {
    ...
    for (int i = 0; neededL1s && (i < ft->M2); i++) {
      ...
      if ((!eflag&&l1st->numFreeHosts >= ft->M1) || (eflag&&l1st->numIdleHosts >= ft->M1)) {
                            ...
                            int &R, int &S, int &Q)
{
  ...
  U = ((N+ft->M1-1)/ft->M1); // U = ceil(N/M1), U = QR + S, 占满了U个M1节点的树
  A=C=ft->M1;
  B=D=0;
...
int
IsolAlgo::recordAllocation(Job* job,
                           vector<int> &l1Allocations, 
                           vector<int> &l2Allocations,
                           vector<PortMask> &jobL1UpPorts,
                           vector<PortMask> &jobL2UpPorts,
                           unsigned int &numLinks)
{
  ...
  int jobId = job->id;
  int origHosts = job->numHosts;
  int numL1s = 0;
  unsigned int numHosts = 0;
  std::vector<int> allocateHosts;
  
  std::vector<int> sleephosts;
  std::vector<int> idlehosts;
  std::vector<int> lasttree_sleephosts;
  std::vector<int> lasttree_idlehosts;

  for (size_t i = 0; i < l1Allocations.size(); i++) {
    unsigned int n = l1Allocations[i];
    if (n > 0) {
      lasttree_idlehosts.clear();
      lasttree_idlehosts.clear();
      SubTree *st = &ft->L1SubTreeByIdx[i];
      ft->setJobNodeAllocation(job, st, n, energy);
      availableHosts -= n;
      numL1s++;
      numHosts += n;
      ...
      // take the first available n bits..., 当考虑能耗时，应该优先分配空闲节点，然后才是睡眠的节点。
      PortMask mask(L1FreeDnPorts[i].len(), 0);
      for (unsigned int j = 0; (n > 0) && (j < L1FreeDnPorts[i].len()); j++)  {
        int hostid = i * ft->M1 + j;
        if (L1FreeDnPorts[i].getBit(j)&&energy->getState(hostid)==idle) {
          mask.setBit(j, true);
          L1FreeDnPorts[i].setBit(j, false);
          L1DnPortsByJob[jobId][i].push_back(j);
          n--;
        
          if (find(allocateHosts.begin(), allocateHosts.end(), hostid)!= allocateHosts.end()) {
            throw "error";
          }
          allocateHosts.push_back(hostid);
          idlehosts.push_back(hostid);
          lasttree_idlehosts.push_back(hostid);
        }
      }
      ...
      if(n > 0){
        for (unsigned int j = 0; (n > 0) && (j < L1FreeDnPorts[i].len()); j++)  {
          int hostid = i * ft->M1 + j;
          if (L1FreeDnPorts[i].getBit(j)&&energy->getState(hostid)==sleepstate) {
            mask.setBit(j, true);
            L1FreeDnPorts[i].setBit(j, false);
            L1DnPortsByJob[jobId][i].push_back(j);
            n--;
            ...
            allocateHosts.push_back(hostid);
            sleephosts.push_back(hostid);
            lasttree_sleephosts.push_back(hostid);
          }
        }
      }
      if(n > 0){
  ...
  if(sleephosts.size() + idlehosts.size() != allocateHosts.size()){
    throw "error";
  }
  
  int notlasttreetotal = allocateHosts.size() - (lasttree_idlehosts.size()+lasttree_sleephosts.size());
  int more_idle_nodes = lasttree_idlehosts.size() + notlasttreetotal - job->numHosts;
  if(more_idle_nodes<0)
    more_idle_nodes = 0;
  
  int more_sleep_nodes = 0;
  if(more_idle_nodes==0){
    more_sleep_nodes = allocateHosts.size() - job->numHosts;
  }else{
    more_sleep_nodes = lasttree_sleephosts.size();
  }
  
  if(sleephosts.size()!= more_sleep_nodes){
    job->length += energy->sleep2run;
    job->haswakeup = true;
    energy->idle_seemas_run_e += energy->sta2power[run] * energy->sleep2run * (idlehosts.size() - more_idle_nodes);
    energy->addtional_idle_e += energy->sta2power[idle] * energy->sleep2run * (idlehosts.size() - more_idle_nodes);
    energy->wakeup_e += energy->sta2power[run] * energy->sleep2run * (sleephosts.size() - more_sleep_nodes);
  }
  ...
  if (!noLinkAlloc) {
    for (size_t l = 0; l < jobL1UpPorts.size(); l++) {
      ...
      if(usedPorts != 0)
        int a=0;
      unsigned long nousedPorts = ~usedPorts; //
      L1FreeUpPorts[l] &= ~usedPorts; // 相当于删除？
  ...
  log << "ADD Job:" << jobId << " hosts:" << vector_int_to_str(allocateHosts, ",") << " links:" << numLinks
...
bool 
IsolAlgo::completeJob(Job* job, int numHosts, vector<int> & deallohosts)
{
  unsigned int numL2Links = 0;
  unsigned int numLinks = 0;
  int jobId = job->id;
  ...
  if (jI != L1DnPortsByJob.end()) {
    ...
    for (lI = (*jI).second.begin(); lI !=  (*jI).second.end(); lI++) {
      ...
      for (pI = (*lI).second.begin(); pI != (*lI).second.end(); pI++) {
        ...
        unsigned int hostid = l*ft->M1 + p;
        if (find(deallohosts.begin(), deallohosts.end(), hostid)!= deallohosts.end()) {
          // cerr << "-F- BUG as the same host " << hostid << "allocated to different jobs" << endl;
          // exit(1);
          throw "error";
        }

        if(!energy->inlock(hostid)&&energy->HostsStateATime[hostid].sta != run){
          throw "error";
        }
        
        if(!energy->inlock(hostid)&&energy->HostsStateATime[hostid].dtime != job->length){
          throw "error";
        }
        
        deallohosts.push_back(hostid);
        numLinks++;
      }
    }
  }
