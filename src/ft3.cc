...
ThreeLevelFatTree::ThreeLevelFatTree(int m1, int m2, int m3, int w2, int w3)
{
  ...
  for (i = 0; i < M3; i++) {
    ...
    L2SubTreeByIdx[i].numIdleHosts = M2*M1;
    ...
    for (j = 0; j < M2; j++) {
      ...
      L1SubTreeByIdx[k].numIdleHosts = M1;      
...
int 
ThreeLevelFatTree::getUnAllocatedSubTrees(SubTree *st, int num, 
                                          vector<int> &idleidxes, vector<int> &sleepidxes, Energy *energy)
{
  int numAlloc = 0;
  
  for (int i = 0; (numAlloc < num) && (i < st->numDnPorts); i++) {
    if(st->dnPortJob[i] == 0&&energy->getState(M1*st->idxInLevel + i)==idle){
      idleidxes.push_back(i);
      numAlloc++;
    }
  }
  if(numAlloc < num){
    for (int i = 0; (numAlloc < num) && (i < st->numDnPorts); i++) {
      // 已经remove的在当前迭代中仍然可能是运行状态。但是其实remove之后，就已经可以分配的，所以这里出问题了
      if(st->dnPortJob[i] == 0&&energy->getState(M1*st->idxInLevel + i) == run){
        throw "error";
      }

      if(st->dnPortJob[i] == 0&&energy->getState(M1*st->idxInLevel + i)==sleepstate){
        sleepidxes.push_back(i);
        numAlloc++;}
    }
  }
  return (numAlloc < num);
...
void
ThreeLevelFatTree::setJobNodeAllocation(Job* job, SubTree *st, int num, Energy *energy)
{
  if (st->level != 1) {
  ...
  vector<int> idleidxes;
  vector<int> sleepidxes;
  if (getUnAllocatedSubTrees(st, num, idleidxes, sleepidxes, energy)) {
    // cout << "-E- setJobNodeAllocation : Sub Tree: " << st->name()
    //      << " could not find " << num
    //      << " free children requested" << endl;
    // exit(1);
  ...
  // for (int f = 0; f < num ; f++) {
  //   int i = freeIdxes[f];
  //   st->dnPortJob[i] = job->id;
  //   st->numFreeHosts--;
  //   jobSubTreePorts[job->id][st].push_front(i);
  // }
  

  for(int f = 0; f < idleidxes.size(); f++){
    int i = idleidxes[f];
    st->dnPortJob[i] = job->id;
    st->numFreeHosts--;
    st->numIdleHosts--;
  ...
  for(int f = 0; f < sleepidxes.size(); f++){
    int i = sleepidxes[f];
    st->dnPortJob[i] = job->id;
    st->numFreeHosts--;
    jobSubTreePorts[job->id][st].push_front(i); 
  ...
  if (st->numFreeHosts < 0 || st->numIdleHosts < 0) {
...
int ThreeLevelFatTree::hostid2subtreeidx(int hostid){
  return hostid/M1;
}
